...
The MAX30001G incorporates a biopotential and impedence measurement unit. Its moden design is available only as waver level packaing which makes in house soldering difficult.
...
- The chip is available individually [Max 30001G ($10)](https://www.maximintegrated.com/en/products/analog/data-converters/analog-front-end-ics/MAX30001G.html) or as evaluation kit ($113).

## Skin Equivalent Circuits

To calibrate bioimpedance measurements a skin or tissue quivalent model shold be built so that representative measurements can be conducted.

There are several impedance models needed for the different impedance measurements:
...
Usually measurements occurs with an oscillator from 1 (or 5) kHz to (100 or) 1MHz.
...
Paco et al [10] describes a skin electrode model and some example valus for a tissue model as well as the electrodes are give here:
...
Here we sumamrize the features and design consideration of the MAX 30001G. This should shorten the read of the datasheet.
