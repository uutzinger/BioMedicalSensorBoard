...
Buffer buffer_i2s;                                                // data from i2s
...
void fillI2SFromBuffer(Buffer& buffer) {
// =======================================================================================
// Stream data from buffer to I2S
// Microcontroller is sourse
// =======================================================================================

  if (buffer.num_chan == NUM_CHAN) {
    size_t bytes_written = i2s_stream.write(const uint8_t* buffer.data,  buffer.bytes_recorded);
  } else if (buffer.num_chan == 1){
...
void setup() {
  ...
  // I2S input buffer
  buffer_i2s   = initBuffer(BYTES_WANTED);
  buffer_i2s.num_chan = NUM_CHAN;
  buffer_i2s.bytes_per_sample = BYTES_PER_SAMPLE;
  ...
  // check the buffers
  if (buffer_i2s.data   == NULL){ LOG_Eln("Failed to allocate input buffer"); }
...
void loop() { 
  ...
  // Obtain samples
  if (buffer_i2s.bytes_recorded < BYTES_WANTED) {
    if (i2s_stream.available()) {
      fillBufferFromI2S(buffer_i2s, BYTES_WANTED);
    }
  }

  // Process and Stream
  if (buffer_i2s.bytes_recorded == BYTES_WANTED) {
    processBuffer(buffer_i2s, buffer_avg);
    if (NUM_CHAN % 2 == 0) {
