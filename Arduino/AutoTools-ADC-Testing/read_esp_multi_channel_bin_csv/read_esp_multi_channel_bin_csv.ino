...
AudioInfo                 info_serial(SAMPLE_RATE/BINSIZE, NUM_CHANNELS, BIT_DEPTH);
AnalogAudioStream  analog_in; // on board analog to digital converter
Bin                       binner;                            // channel averager
ConverterStream<int16_t>  binned_stream(analog_in, binner);     // pipe the sound to the averager
...
void setup(void) {
  ...
  analog_in.begin(adcConfig);
   ...
   // Configure binning
  binner.setChannels(NUM_CHANNELS );
  binner.setBits(BIT_DEPTH);
