...
#define BUFFER_SIZE_FACTOR (1024/ (NUM_CHANNELS*BINSIZE*BIT_DEPTH/8)) // 
#define ADC_BUFFER_SIZE (NUM_CHANNELS*BINSIZE*BUFFER_SIZE_FACTOR)     // Total number of samples (not bytes), needs to be divisible by 4 // 3*3*96 = 864
#define BUFFER_SIZE ADC_BUFFER_SIZE*BIT_DEPTH/8               // Number of bytes for processing buffer

// Sampling rates
#define SAMPLE_RATE 4096 // samples per channel per second
#define ADC_SAMPLE_RATE    SAMPLE_RATE*NUM_CHANNELS // actual sampling rate of ADC
// BAUD rate required: SAMPLE_RATE * (NUM_CHANNELS * approx 6 chars + 2 EOL) * 10 
...
AudioInfo                 info_serial(SAMPLE_RATE/BINSIZE, NUM_CHANNELS/2, BIT_DEPTH);
AnalogAudioStream         analog_in; // on board analog to digital converter
ChannelBinDiff            bindiffer;                               // channel averager and subtracter
ConverterStream<int16_t>  binned_stream(analog_in, bindiffer);     // pipe the sound to the converter
CsvOutput<int16_t> serial_out(Serial, NUM_CHANNELS/2, BUFFER_SIZE);  // serial output
StreamCopy         copier(serial_out, binned_stream, BUFFER_SIZE); // stream the decimated output to serial port
...
void setup(void) {
  ...
  analog_in.begin(adcConfig);
   ...
   // Configure binning
  bindiffer.setChannels(NUM_CHANNELS );
  bindiffer.setBits(BIT_DEPTH);
