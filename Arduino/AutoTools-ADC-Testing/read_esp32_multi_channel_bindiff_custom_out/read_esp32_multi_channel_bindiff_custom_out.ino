...
#define M_ADC_SAMPLES        (M_NUM_CHANNELS*M_BIN*2) // number of samples over all channels
#define M_ADC_BUFFER_SIZE    (M_ADC_SAMPLES*sizeof(int16_t)) // number of bytes over all channels
#define M_CSVOUT_BUFFER_SIZE (M_NUM_CHANNELS*2)*sizeof(int16_t) // number of bytes at output of converter going into CsvOutput

#define M_ADC_BIT_WIDTH      12
#define M_BIT_WIDTH          16

#define M_SERIAL_SAMPLE_RATE 120 // samples per channel
#define M_ADC_SAMPLE_RATE    M_SERIAL_SAMPLE_RATE*M_BIN // samples per channel 23040
...
void setup() {
  ...
  converter.setBinSize(M_BIN);
  converter.setAverage(true);
...
void loop() {
  ...
  for (int i = 0; i < samples_read/M_NUM_CHANNELS/2; i++) {
    for (int j = 0; j<M_NUM_CHANNELS/2; j++) {
