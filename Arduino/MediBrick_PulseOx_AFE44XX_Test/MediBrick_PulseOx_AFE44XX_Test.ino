...
// For ESP32 Thing Plus C
// #define RESET_PIN    19  // active low, hardware reset, use if SPI not longer works with the sensor

// For feather ES32 S3
#define RESET_PIN    10  // active low, hardware reset, use if SPI not longer works with the sensor

// Same on both boards
#define DRDY_PIN     13  // data ready interrupt from AFE
#define CS_PIN        6  // chip select, active low, needed for SPI, you can hardwire this pin or activate when you have multipole SPI devices
#define PDALARM_PIN  12  // Photodiode Alarm pin, this can also be used to show activity of the AFE measurement process
#define LEDALARM_PIN  9  // LED Alarm pin, this can also be used to show activity of the AFE measurement process
#define DIAGEND_PIN  11  // This can be used to wait until diagnosis is finished, but it is usually not necessary
#define PWDN_PIN      5  // power down pin, active low, you can hardwire for always on and turn off LED, Photodiode and other hardware with software
...
bool    afe44xx_reporting_simple = false; // continously report IR and red measurements on serial port
/* Receiver Stage  */
...
void setup() {
  ...
  LOGI("AFE: Setting pins for RESET, PD&LED ALARM, DIAGEND");
  ...
  LOGI("AFE: starting continous measurements");
...
void loop() {
  ...
  if (afe44xx.afe44xx_data_ready) {
    ...
    if (afe44xx_reporting) {
      snprintf(tmpStr, sizeof(tmpStr), "IR: %9i\tRed: %9i\tIR_a: %i\tRed_a: %9i\t S\\s: %.1f", irValue, redValue, irAmbientValue, redAmbientValue, samplingRate); 
      Serial.println(tmpStr);
    }
    if (afe44xx_reporting_simple) {
      snprintf(tmpStr, sizeof(tmpStr), "%9i, %9i", irValue, redValue); 
      Serial.println(tmpStr);
    }
  }
...
void helpMenu() {
  Serial.println("================================================================================");
  Serial.println("| AFE4490 Texas Instruments Analog Front End for Clinical 2 ch Pulse Oximeter  |");  
  ...
  Serial.println("| l: 3 info                     l3      | l: 98 continous data simple          |");
