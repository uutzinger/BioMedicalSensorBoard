...
The evaluation board from AD includes the reference design describing an analog front end to measure low impedances.
...
### Analog Frontend for Bioimpedance (Munoz et al [2])

For bioimpedance measurements the following analog front end elements should be usedd: a high-pass filter (HPF), a voltage-to-current converter (VCC), and an instrumentation amplifier (INA).
...
| Design        | $C_{HP}$ | $R_{HP}$ | $RFB$ | $R_V{CC_{current}}$ | $R_{VCC_{protect}}$ | $R_{INA_{GAIN}}$ | $R_{REFGEN}$ | $R_{AD_{IN}}$ | $R_{AD_{FB}}$ | VCC pos
|---            |---   |---    |---   |---            |---              |---            |---       |---      |---      |---
...
Formula to compute impedance from measurements.
...
There are several bioimpedance models to measure body impedance:
